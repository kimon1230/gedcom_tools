...
A minor rather than a patch release: `export` and `stats` now report years for dates they previously reported as unknown, and `export --redact-living` publishes a small number of people it used to redact.
...
- **`export --redact-living` now publishes a small number of individuals it previously redacted.** A year recovered from date text is reported in `birth_year`/`death_year` but is only allowed to influence the living/not-living decision when the text is a date and nothing else — `30 November 1989`, `12/2/1882` and the range `1801-1875` qualify; `Census 1900 record`, `Reg. 1823 vol II` and `1901, 1902, 1903, 1904` do not. The distinction matters because the first four digits of a free-text note are as likely to be an archive reference as a birth year, and acting on one would publish a living person. On a large private test tree this moved a handful of individuals — people whose date ranges prove they are long dead — out of redaction, while still withholding those whose years could not be trusted. Individuals whose only date is genuinely unreadable are redacted exactly as before
