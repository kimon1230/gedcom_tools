...
A minor rather than a patch release: `export` and `stats` now report years for dates they previously reported as unknown, and `export --redact-living` withholds a small number of people it used to publish.

### Added
- **`W035` also names dates the parser reads *wrong*, not only ones it cannot read at all.** `Reg 1823` becomes a date whose month is "REG", `3/1990` becomes the dual year 3 rather than March 1990, and `1056/1060` becomes a dual year rather than the range it was meant to be — none of them look amiss in a report, and all three silently feed a wrong year to the age and chronology checks. A genuine dual date still does not warn: `1750/51` spans one year boundary, which is what separates a Julian/Gregorian transition year from a range written the wrong way. Neither does a date in correct form whose *year* is merely implausible — `25 DEC 9999` has a year problem, not a format problem. On the `royal92.ged` reference file this finds two non-conformant dates the warning previously missed
- **`W035` checks every dated event on a record, not just the first.** Dates were reached one path at a time, and that returns a single match, so the second `BIRT` of a tree merged from two sources was checked by nothing at all. Marriage dates needed a second walk of their own
- **`validate` now reports `W035`, naming dates it could not read as dates.** GEDCOM asks for a three-letter month; a date written `30 November 1989`, `12/2/1882` or `10 JAN` is free text to the parser, so the year behind it is recovered heuristically rather than read. Without this the recovery is silent, and a four-digit run in a free-text note is as often an archive reference as a year. The message names the offending value and, where the month is spelled out, the abbreviation to use — `12/2/1882` is ambiguous between 12 February and 2 December so no rewrite is offered, and `Christmas 1901` gets the generic rule. Checked on every date whose year `export` and `search` recover — `BIRT`, `CHR`, `BAPM`, `DEAT`, `BURI` and `MARR` — so the warning set matches the set of dates that can produce a guessed year. A single parenthesised `(DATE_PHRASE)` is valid GEDCOM and does not warn, but a value carrying two of them, such as `(1850) and later (see note)`, does. Capped at the first 10 like the other per-line warnings, with the dropped count carried in `summary.suppressed` so `total_warnings` still reports what the file contains

### Fixed
- **`export --redact-living` published individuals it should have withheld, in three separate ways.** A non-Gregorian date decided liveness on its raw year, so `@#DFRENCH R@ 1 VEND 230` — a date in 2021 — read as an age of roughly 1796 and published a living person; GEDCOM 5.5.1 Hebrew and French Republican years are now converted to the Gregorian scale before any decision is made on them, while `birth_year`/`death_year` still report the year as the file writes it. A living-status tag was matched case-sensitively, so a hand-edited or non-conforming `_lvg` matched nothing and the strongest do-not-publish signal in the format was discarded; tags are now case-folded. And the liveness bound stopped at the first of `BIRT`, `CHR` and `BAPM` that parsed, so an ancient or mistranscribed birth year overrode a later christening on the same record; it now takes the latest of the three, which can only withhold more. The reported `birth_year` still takes `BIRT` first and is unchanged. The conversion goes through `convertdate` rather than ged4py's own Julian Day: ged4py feeds its GEDCOM month index (`TSH`=1) into a library that numbers Hebrew months from Nisan, so its answer is about six months out — Tishrei–Tevet a year high, and Nisan–Elul a year LOW, which is the direction that publishes someone. The same conversion fixes `validate`, which previously compared a Hebrew year against a Gregorian one and so raised `E011` on records mixing a Hebrew birth date with a Gregorian death date
- **`.gitignore` matched only lowercase `*.ged`**, so a real family tree saved as `MyTree.GED` — the extension PAF, Legacy and Family Tree Maker write — was stageable by `git add .`. The rule is now case-folded and also covers the `.ged.bak`/`.ged.gz` backup forms
- **`birth_year` and `death_year` came back `null` for dates that plainly contain a year** ([#20](https://github.com/kimon1230/gedcom_tools/issues/20)). ged4py parses only three of the twelve full month names — May, June and July — so `2 DATE 30 November 1989` is not a date to it but free text, and the same is true of `12/2/1882` and of a day-and-month with no year. All four date helpers returned early on that case, so `export --to json` emitted the date string next to a null year, and `stats` on a file written that way printed "No date data available" with an empty timeline. The year, month and precision are now recovered from the text. On the 3,010-person `royal92.ged` reference file this adds one birth to the January column of the month histogram and changes nothing else
- **A recovered year is now range-checked, so an identifier in a note cannot become a birth year.** `2 DATE vol 6789 p. 4` reported `birth_year: 6789` and put the record in a `6700s` bucket in the `stats` century breakdown. Four-digit runs are now filtered to plausible years before one is chosen, so the first *plausible* run wins rather than the first run — `ref 6789 b. 1850` reports 1850, and a date with no plausible run reports none. The year, the upper bound and the precision beside them all use the same filter, so a date can no longer report a null year while claiming `partial` precision
- **The year recovery reaches `compare`, `duplicates`, `relationship` and `isolated` as well as `export`, `stats` and `search`.** `compare` and `duplicates` read `birth_year`/`death_year` for their blocking keys and their Birth Year score, so records that carried no year before now take part in year-keyed matching passes they previously sat out. This finds pairs it could not find before; it also means a citation year such as `Reg. 1823 vol II` is now a matching year. `relationship` and `isolated` display the recovered year
- **A `DATE` carrying `CONT` continuations echoed the rest of the record into the validation report.** ged4py joins continuation lines into the date value, so a birth date followed by an address and an identifier printed all three — into a report that has no `--redact-living` and is routinely pasted into bug trackers. Only the date line itself is echoed now
- **A filename could forge the tool's own verdict.** POSIX allows a newline in a filename and the report prints it in the header, so a file named `tree\n✓ Valid\nx.ged` printed `✓ Valid` at column zero of its own validation report. `sanitize_error` deliberately keeps newlines so a wrapped exception still reads as paragraphs, so single-line output now uses a stronger scrub that flattens them; the same applies to the JSON `file` and `filename` fields
- **Issue text is now bounded.** ged4py quotes the entire offending source line into its exception text, and a GEDCOM line may be 255 bytes before any continuation. That reached stdout with no `--verbose`
- **`W035`'s suppression line now gives the count**, like every other capped code. It was emitted at the eleventh date, when the total was still unknown, so it could only say "more were suppressed" while `W002`/`W003`/`W032` said "1,234 more"
- **The month-abbreviation hint was lost on a date padded with escape sequences.** The hint was gated on an offset into the raw text compared against a window into the scrubbed text — two different coordinate systems, so any stripped byte shifted the answer
- **A mixed-calendar range dropped its Gregorian bound.** `BET @#DHEBREW@ 1 TSH 5600 AND 2010` read as 1840 — an age of roughly 186 — and published a living person. Every half of a range is now converted, and a half that will not convert makes the whole value unreadable rather than leaving the other half standing alone as a bound the file never gave
- **A `BC` year was read as `AD`.** ged4py reports the year and a separate `bc` flag; only the flag says which. `100 B.C.` beside a death in 50 B.C. therefore invented a reversed lifespan
- **An absurd calendar year aborted the whole run.** `@#DFRENCH R@ 1 VEND 3000` raised `ValueError` from the conversion library and a 400-digit year raised `OverflowError`; neither was caught, so one unreadable date ended validation for the entire file
- **`validate` held one Python integer per parenthesised `DATE` line for the length of the run** — around 65 bytes each where the line map beside it uses 8, and roughly 3 GB on a file at the 500 MB input ceiling. It is now the same compact array, searched by bisection
- `stats` silently dropped June and July from the birth-month histogram even for dates ged4py parses correctly. The month lookup was keyed on three-letter codes while ged4py returns `.date.month` verbatim, so `3 June 1900` yielded no month while the precision breakdown for the same record reported a complete date. Found while fixing the above; it is an independent defect that predates it

### Changed
- **`validate` takes the strict reading of a recovered year, so it does not fail a file on an archive reference.** Age and chronology checks (`W020`-`W023`, `E011`, `E012`) act on the year, and a four-digit run in a free-text date note is as often a reference number as a date — `2 DATE Reg. 1823 vol II` beside a real death in 1950 would otherwise report a 127-year lifespan the file never claimed. Those checks now only see a year when the date text is a date and nothing else. They deliberately do **not** inherit the plausibility floor that guards free text: a floor written to reject a four-digit run in a note has no business rejecting `2 DATE 1 JAN 0950`, and applying it silently dropped one `W020` and one `W025` from the `royal92.ged` reference file. `stats` and `export` keep reporting the looser reading, which is the point of the fix above
- `search` date filters now match records whose date is written in a non-standard form, since the year is recovered the same way `export` recovers it — `--birth-year 1900` matches a record whose only birth date is `Census 1900 record`. This is the loose reading on purpose: search filters what the file reports, and that year is what `export` reports for the same record
- **`export --redact-living` no longer treats any text in a `BURI/DATE` line as proof of death.** `bool(burial_date)` was the test, so `1 BURI / 2 DATE (pre-need plot)` — a note about a plot someone bought while alive — published their full record, while a `BURI` with no `DATE` line at all was correctly redacted. The decision keyed on whether the line happened to carry text rather than on whether there was burial evidence. Burial now goes through the same gate as birth and death: a year, read strictly. This redacts slightly more than before
- **`export --redact-living` now withholds a small number of individuals it previously published.** A year recovered from date text is reported in `birth_year`/`death_year` but is never allowed to influence the living/not-living decision. The recovery cannot separate a date from a citation shaped like one — `12.1823.4` (volume, year, page) has the same token profile as `12/2/1882`, so any rule that accepts the second accepts the first, and acting on a wrong one publishes a living person. Unknown therefore means living: `30 November 1989`, `12/2/1882` and the range `1801-1875` are reported but redacted, as `Census 1900 record` and `Reg. 1823 vol II` already were. The cost is over-redacting a few people whose free-text dates prove them long dead; the alternative was publishing living ones. Records whose dates ged4py parses normally are unaffected
...
- Names and places from the file were printed to the terminal unfiltered, so a crafted GEDCOM could emit window-title escapes, clear the screen, or use bidi overrides to reorder what the user saw — enough to fake a `✓ Valid` verdict. Terminal output is now stripped of control sequences other than the tool's own colours. Redirected and `-o` output is deliberately left untouched, since a file has nothing to interpret them. The stripping now also covers DEL and the C1 block `\x80`-`\x9f`, which it previously passed through — U+009B is the 8-bit form of `ESC[`, so a terminal in 8-bit mode read it as a control-sequence introducer and the one character that most needed removing was surviving the function that removes escape introducers. NBSP and everything above it are still ordinary text. A newline is still passed through deliberately: stripping it would stop a multi-line exception forging a second `Error:` line, but it is also how every legitimately wrapped message renders
...
- `filter` did not see six kinds of line that ged4py — and therefore `validate` — reads as a whole record: a leading space or a leading tab before the level number, a level written with three or more digits, no space at all between the level and the xref, and a hyphen anywhere in the tag, as in `0 @I5@ FOO-BAR` or `1 _MY-TAG x`. Its line pattern was hand-written and had drifted from the one `GedcomReader` actually lexes with, so any line it failed to match was kept with an empty tag and copied to the output verbatim. Every strip and subtree option was affected — `--strip-notes`, `--strip-sources`, `--strip-multimedia`, `--strip-custom-tags`, `--strip-tag` and `--subtree` all passed straight over such records — while the summary counted them among the records removed, so a `--subtree` extraction could report one individual and ship five. The pattern is now derived from ged4py's own and matched against the same preprocessed line, so the two grammars cannot drift apart. **Two kinds of line that used to parse no longer do**: an xref whose first character is not alphanumeric (`0 @.X@ INDI`) and a line using tabs as its internal delimiters (`0<TAB>@I1@<TAB>INDI`) are no longer record boundaries. ged4py never treated them as records, and they now attach to the preceding record as its children, which means `--subtree` on that preceding xref carries them along and `--subtree @.X@` finds nothing
- A file's own `1 CHAR` header chose the codec used to decode it, and `filter` and `convert` decode the whole file before splitting it into lines — so the header decided where the line boundaries fell. `1 CHAR UTF-7` is enough to exploit this: `+AAo-` is plain ASCII that decodes to U+000A, so a NOTE value containing no line break at all became two extra level-0 lines, and `filter --strip-notes` wrote out an individual record that was never in the input. Auto-detection now honours only codecs that cannot invent a line terminator (UTF-8/16, the ISO-8859 and Windows code pages, Mac Roman, KOI8, ANSEL); anything else is refused with `Cannot determine source encoding`, and an explicit `--from utf-7` still processes the file. This applies only to `filter` and `convert`, which decode the file themselves — `validate` and `stats` read through ged4py, which splits on bytes before decoding, and will still accept and report on the same file
...
- Two more places printed an exception straight to the terminal without that stripping: the `Warning: Could not detect encoding` line in `stats` and the byte-identical one in `languages`. The exception behind it is either a parser error quoting the offending GEDCOM line verbatim or an OS error quoting the input path, so both carry text the file or the filename controls. `validate` already handled the same case correctly
...
### Added
- `filter` gained `--from CODEC`, matching `convert`: it overrides source encoding detection with any Python codec name and skips header parsing entirely, so a file whose `1 CHAR` value is missing, wrong, or now refused by auto-detection can still be filtered. A `--from` the bytes will not survive is reported as a usage error rather than a bare `UnicodeDecodeError`. Both this option and the stricter auto-detection behind it are specific to `filter` and `convert`, the two commands that decode the file themselves; `validate` and `stats` go through ged4py and are unchanged
...
- The full `S` (flake8-bandit) ruleset is now enabled, replacing the single `S110` rule that was turned on earlier for a specific bug. It found exactly one thing in `src/` — a `assert` used as a mypy narrowing guard in `languages`, which keeps its `assert` and gains a `noqa`, since converting it to a raise would change behaviour to satisfy a linter. In `tests/`, `S101` and `S108` are ignored wholesale: pytest is built on bare asserts, and the five `S108` hits are inert `"/tmp/test.ged"` string literals used as JSON data, one of which asserts the cache directory is *not* under `/tmp`. The three remaining hits — two `subprocess` calls with fixed argv and the seeded RNG behind the GEDCOM line-parser fuzz — carry inline `noqa`s with reasons
- Dependabot now proposes Python dependency updates as well as GitHub Actions ones. This is a maintenance convenience, **not** new security coverage: Dependabot *alerts* are a repository setting and were already enabled, so CVE notification was never missing. Expect most of the resulting PRs to propose widening the deliberately pinned bounds in `pyproject.toml`
